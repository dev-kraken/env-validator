...
class ValidateEnvCommand extends Command
{
    ...
    {
        // Get environment variables directly from all sources
        $env = array_merge(getenv(), $_ENV, $_SERVER);
        $keys = $this->option('keys');

        // Ensure $keys is always an array of strings
        $keysArray = is_array($keys) ? $keys : [];

        // Cast $env to the expected type
        /** @var array<string, mixed> $typedEnv */
        $typedEnv = $env;
